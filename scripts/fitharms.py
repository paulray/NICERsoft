...
def compute_phist(phases, nbins=200):
    h, edges = np.histogram(
        phases, bins=np.linspace(0.0, 1.0, nbins + 1, endpoint=True)
    )
    return edges[:-1], h
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Fit a set of pulse phases to harmonics"
    )
    parser.add_argument(
        "evname", help="Input event file (must have PULSE_PHASE column)"
    )
    parser.add_argument(
        "--white",
        help="Replace phases with white random numbers, for testing",
        action="store_true",
    )
    parser.add_argument(
        "--txt", help="Assume input file is .txt instead of FITS", action="store_true"
    )
    parser.add_argument(
        "--showcomps",
        help="Show individual components of harmonic fit on plot",
        action="store_true",
    )
    ...
    parser.add_argument(
        "--numharm",
        help="Max harmonic to use in analysis (1=Fundamental only)",
        default=4,
        type=int,
    )
    parser.add_argument(
        "--numbins", help="Number of bins for histograms", default=200, type=int
    )
    parser.add_argument(
        "--emin", help="Minimum energy to include (keV)", default=0.25, type=float
    )
    parser.add_argument(
        "--emax", help="Maximum energy to include (keV)", default=12.0, type=float
    )
    ...
    # Filter on energy
    idx = np.where(
        np.logical_and(en > int(args.emin * 100), en < int(args.emax * 100))
    )[0]
    ph = ph[idx]
    en = en[idx]
    ...
    ax.step(
        np.concatenate((bins, np.ones(1))),
        np.concatenate((phist, phist[-1:])),
        color="k",
        where="post",
    )
    ax.set_xlim(0.0, 1.0)
    ...
    if args.showcomps:
        for k in range(len(c)):
            ax.plot(
                ...
                evaluate_fourier(n, c, s, nbins, k=k),
                ls="--",
            )
    ...
    if exposure:
        log.info(
            "Pulsed counts = {0:.3f}, count rate = {1:.3f}+/-{2:.4f} c/s".format(
                pcounts, pcounts / exposure, pcounts_err / exposure
            )
        )
        log.info(
            "Total rate = {0:.3f} c/s, Unpulsed rate = {1:.3f} c/s".format(
                n / exposure, n / exposure - pcounts / exposure
            )
        )
    ...
    axs[0].set_title(
        "NumHarm = {0}, Chisq = {1:.2f}, DOF = {2}".format(args.numharm, chisq, ndof)
    )
    ax.grid(1)
