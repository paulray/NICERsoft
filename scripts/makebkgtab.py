...
parser = argparse.ArgumentParser(description = 'Construct BKG table from a pipe directory')
parser.add_argument("pipedirs", help="Input _pipe directory process (should be a blank field BKG observation)", nargs='+')
...
#    file=outfile)    
    ...
    for curgti in gtitable:
        ...
        for chmet in chunkmets:
            ...
            lon[i] = mktable['SAT_LON'][chidx].min() # Use min() here to prevent problems when LON wraps
