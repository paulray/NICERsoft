...
------------
                  ...
                  ------    ---------------------    ---------------------    --------------""".format(
    dt.day, mnames[dt.month], dt.year, dt.hour, dt.minute, dt.second
)

parser = argparse.ArgumentParser(
    description="Compute SAA and polar horn entry/exit times."
)
...
""".format(
        accesses[durs.argmin()],
        tstarts[durs.argmin()],
        tstops[durs.argmin()],
        durs.min().value,
        accesses[durs.argmax()],
        tstarts[durs.argmax()],
        tstops[durs.argmax()],
        durs.max().value,
        durs.mean().value,
