...
parser.add_argument(
    "--guessobj", help="Try to guess object from directory name", action="store_true"
)
parser.add_argument(
...
parser.add_argument(
    "--filtratio",
    help="Filter PHA/PHA_FAST ratio (argument is ratio to cut at)",
    type=float,
    default=1.4,
)
parser.add_argument(
...
parser.add_argument(
    "--readovs",
    help="Filters events with overshoot > input number",
    nargs="*",
    type=float,
    default=None,
)
...
if args.bkg:
    if hkmet is None:
        ...
        if eventovershoots is not None:
            ...
            figure4 = bkg_plots(
                etable, data, gtitable, args, mktable, data.eventshoottable
            )
        else:
            figure4 = bkg_plots(
                etable, data, gtitable, args, mktable, data.hkshoottable
            )
...
if args.eng:
    ...
    if args.save:
        ...
        if args.filtall:
            figure1.savefig(
                "{0}_eng_clean_{1:.1f}-{2:.1f}keV.png".format(
...
if args.sci:
    ...
    if args.save:
        ...
        if args.filtall:
            figure2.savefig(
                "{0}_sci_clean_{1:.1f}-{2:.1f}keV.png".format(
                    basename, args.emin, args.emax
                ),
                dpi=100,
            )
...
if args.map:
    ...
    if eventovershoots is not None:
        figure3 = cartography(
            hkmet, eventovershoots, args, eventundershoots, filttable, mktable, gtitable
        )
    else:
        figure3 = cartography(
            hkmet, hkovershoots, args, hkundershoots, filttable, mktable, gtitable
        )
