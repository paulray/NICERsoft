...
parser.add_argument("name", help="SIMBAD Source name for query (must be exact). If --ra and --dec are provided, the query is ignored", type=str)
parser.add_argument("countrate", help="Expected NICER count rate of your target", type=float)
parser.add_argument("near_srcs", help="Text file with RA (hms), DEC (dms), and estimated NICER count-rates of nearby sources", type=str)
...
parser.add_argument("--step",help="Number of grid points to search in RA/DEC", type=float, default=100.0)
parser.add_argument("--saveplot",help="Saves plot instead of display",  action='store_true')
...
if not path.isfile(VignetingFile):
    log.error('This file nicer_vignetting.dat is missing. Check the {} directory'.format(path.abspath(datadir)))
    exit()
else:
...
SRCposition  = SkyCoord(ra=NearbySources[0], dec=NearbySources[1], unit=(u.hourangle,u.deg))   ### Change SRCposition to NearbySourcesPOS
...
print("Target S/N at Nominal Pointing " + str(SignaltoNoise(PSRcountrate,SRCscaleRates,INSTbkgd,EXPtime)))
print("Target count rate at Nominal pointing = " + str(PSRcountrate) + " cts/sec")
print("Total count rate from nearby sources at Nominal pointing = " + str(np.sum(SRCscaleRates)) + " cts/sec")
...
for i in DeltaRA:
    for j in DeltaDEC:
        ...
        SRCcountrateScaled = ScaledCtRate(SRCseparation.arcmin,NearbySourcesCR,EffArea,OffAxisAngle)
...
print("Target count rate at Optimal pointing = " + str(PSRrates[OptimalPointingIdx]) + " cts/sec")
print("Total count rate from nearby sources at Optimal pointing = " + str(SRCrates[OptimalPointingIdx]) + " cts/sec")
...
print("corresponds to offset of " + str(AngSeparation(PSRposition,SkyCoord(ra=sampleRA[OptimalPointingIdx]*u.degree, dec=sampleDEC[OptimalPointingIdx]*u.degree)).arcmin) + " arcmin")
print("----------------------------------------------------------------------")
...
plt.plot(sampleRA[OptimalPointingIdx], sampleDEC[OptimalPointingIdx], marker='o', color='red', linestyle='')
