...
parser.add_argument(
    "--useftools", help="Use FTOOLS for filter and merge", action="store_true"
)
...
parser.add_argument(
    "--eng", help="Makes some nice engineering plots", action="store_true"
)
parser.add_argument(
    "--bkg", help="Display background diagnostic plots", action="store_true"
)
parser.add_argument("--filtswtrig", help="Filter SW TRIG events", action="store_true")
parser.add_argument(
    "--filtovershoot", help="Filter OVERSHOOT events", action="store_true"
)
parser.add_argument(
    "--filtundershoot", help="Filter UNDERSHOOT events", action="store_true"
)
parser.add_argument(
    "--filtratio", help="Filter PI/PI_FAST ratio using trumpet cut", action="store_true"
)
parser.add_argument(
    "--filtall",
    help="Filter SWTRIG, UNDERSHOOT and OVERSHOOT events",
    action="store_true",
)
parser.add_argument(
    "--emin", help="Minimum energy (keV) to keep", default=-1.0, type=float
)
parser.add_argument(
    "--emax", help="Minimum energy (keV) to keep", default=-1.0, type=float
)
parser.add_argument(
    "--tskip", help="Seconds to skip at beginning of data", default=0.0, type=float
)
parser.add_argument(
    "--lcbinsize", help="Light curve bin size (s)", default=1.0, type=float
)
parser.add_argument(
    "--filterbinsize",
    help="Bin size for Count rate and Overshoot rate filtering (s)",
    default=16.0,
    type=float,
)
parser.add_argument(
    "--pi", help="Force use of internal PHA to PI conversion", action="store_true"
)
...
parser.add_argument(
    "--foldfreq",
    help="Make pulse profile by folding at a fixed freq (Hz)",
    default=0.0,
    type=float,
)
parser.add_argument(
    "--nyquist", help="Nyquist freq for power spectrum (Hz)", default=100.0, type=float
)
parser.add_argument(
    "--map", help="Creates a map with overshoots and undershoots", action="store_true"
)
...
parser.add_argument(
    "--powspec",
    help="Display power spectrum (replaces ratio plot)",
    action="store_true",
)
...
parser.add_argument(
    "--writebkf",
    help="Write useful rates for background filtering to FITS file",
    action="store_true",
)
parser.add_argument("--applygti", help="Read GTI from provided FITS file", default=None)
parser.add_argument(
    "--extraphkshootrate",
    help="Compute HK shoot rates from a single MPU",
    action="store_true",
)
parser.add_argument(
    "--eventshootrate",
    help="Gets over/undershoot rates from the events",
    action="store_true",
)
parser.add_argument(
    "--interactive", help="TEST FOR INTERACTIVE LC", action="store_true"
)
parser.add_argument(
    "--gtirows", help="Select GTI rows", nargs="*", type=int, default=None
)
parser.add_argument(
    "--merge",
    help="when using a merged file, the OBSID keyword in header is updated (for plotting purposes)",
    action="store_true",
)
parser.add_argument(
    "--allspec",
    help="Makes a figure will a spectrum for each DET_ID",
    action="store_true",
)
parser.add_argument(
    "--alllc",
    help="Makes a figure will a lightcurve for each DET_ID",
    action="store_true",
)
parser.add_argument(
    "--keith",
...
if np.logical_or(args.obsdir is not None, args.infiles is not None):
    if args.obsdir is not None:
        ...
        gtitable = Table.read(args.infiles[0], hdu="GTI")
        if "TIMEZERO" in gtitable.meta:
            log.info(
                "Applying TIMEZERO of {0} to self.gtitable in NicerFileSet".format(
                    gtitable.meta["TIMEZERO"]
                )
            )
        ...
        log.info(
            "TSTART {0}  TSTOP {1} (Span {2} seconds)".format(
                etable.meta["TSTART"],
                etable.meta["TSTOP"],
                etable.meta["TSTOP"] - etable.meta["TSTART"],
            )
        )
        log.info(
            "DATE Range {0} to {1}".format(
                etable.meta["DATE-OBS"], etable.meta["DATE-END"]
            )
        )
...
if args.merge:
    log.info(
        'Overwriting the header of outfile {0} by "{1}"'.format(
            path.basename(args.infiles[0]), path.dirname(args.infiles[0])
        )
    )
...
if args.eng:
    ...
    if args.save:
        ...
        if args.filtall:
            figure1.savefig(
                "{0}_eng_{1:.1f}-{2:.1f}keV.png".format(basename, args.emin, args.emax),
                dpi=100,
            )
        else:
...
if args.sci:
    ...
    if args.save:
        ...
        if args.filtall:
            figure2.savefig(
                "{0}_sci_{1:.1f}-{2:.1f}keV.png".format(basename, args.emin, args.emax),
                dpi=100,
            )
        else:
