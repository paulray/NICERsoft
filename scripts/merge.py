...
parser.add_argument(
    "infile", help="list of event files or text file with list", nargs="+"
)
...
parser.add_argument(
    "--filterbinsize",
    help="Bin size for Count rate filtering (sec, default=16.0)",
    type=float,
    default=16.0,
)
parser.add_argument(
...
if args.par and args.orb:
    ...
    if len(all_evfiles) != len(all_orbfiles):
        ...
        log.info(
            "Orbit Files to use: {0}".format("\n      " + "\n      ".join(all_orbfiles))
        )
else:
    if (args.par and not args.orb) or (args.orb and not args.par):
        log.error(
            "You need both --par and --orb for the phase calculations! Exiting..."
        )
        exit()
...
if args.cut:
    ...
    cmd = [
        ...
        "eventsout=none",
        "imgfile=none",
        "phafile={0}".format(phafile_cut),
        "fitsbinlc={0}".format(lcfile_cut),
        "binlc={}".format(args.lcbinsize),
        "regionfile=none",
        "timefile=none",
        "xcolf=RAWX",
