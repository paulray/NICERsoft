...
def light_curve(
    ...
    cod = axes.hist(
        phases, bins=bins, weights=weights, density=True, histtype="step", ec=ec
    )[0]
    ...
    if weights is None:
        ...
        for i in range(nmc):
            ...
            err[:, i] = np.histogram(phases, bins=bins, weights=rweights, density=True)[
                0
            ]
    ...
    if template is not None:
        ...
        resids = cod / model - 1
        axes_resid.errorbar(
            bcs, resids, yerr=err / model, color="green", ls=" ", marker="o", alpha=0.5
        )
...
class InteractiveFitter(object):
    ...
    def welcome(self):
        ...
        print(
            "After (left button) each drag, the plot will refresh with the current template."
        )
    ...
    def do_fit(self, doshow=False):
        ...
        fitter.fit(
            estimate_errors=self.errors, use_gradient=True, unbinned=self.unbinned
        )
        print(fitter)
        self.fig = pl.figure(self.fignum)
        self.ax = pl.gca()
        light_curve(
            self.phases, weights=self.weights, nbins=self.nbins, template=template
        )
        if doshow:
            pl.show()
        self.fitter = fitter
    ...
    def write_profile(self, outfile, nbin, integral=True, suppress_bg=True):
        if not hasattr(self, "fitter"):
            print("Must do fit first!")
            return
        self.fitter.template.write_profile(
            outfile, nbin, integral=integral, suppress_bg=suppress_bg
        )
...
if __name__ == "__main__":
    ...
    if False:
        ...
        intf = InteractiveFitter(
            phases,
            nbins=options.nhistbins,
            weights=weights,
            errors=options.errors,
            unbinned=options.unbinned,
        )
        ...
        if options.prof is not None:
            ...
            if len(dname) > 0 and not os.path.exists(dname):
                raise IOError(
                    "Specified directory %s does not exist!" % (os.path.dirname(out))
                )
            prof = options.prof
        else:
            prof = "itemplate"
